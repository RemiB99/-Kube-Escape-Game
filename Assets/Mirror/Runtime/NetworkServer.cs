...
namespace Mirror
{
    ...
    public static class NetworkServer
    {
        ...
        public static ConnectionToClient localConnection { get; private set; }
        ...
        /// </summary>
        public static Dictionary<int, ConnectionToClient> connections = new Dictionary<int, ConnectionToClient>();
        ...
        public static bool AddConnection(ConnectionToClient conn)
        {
        ...
        static bool SendToObservers<T>(NetworkIdentity identity, T msg, int channelId = Channels.DefaultReliable) where T : IMessageBase
        {
            ...
            if (identity != null && identity.observers != null)
            {
                ...
                using (PooledNetworkWriter writer = NetworkWriterPool.GetWriter())
                {
                    ...
                    {
                        result &= ConnectionToClient.Send(connectionIdsCache, segment, channelId);
                    }
        ...
        public static bool SendToAll(int msgType, MessageBase msg, int channelId = Channels.DefaultReliable)
        {
            ...
            // send to all
            bool result = true;
            foreach (KeyValuePair<int, ConnectionToClient> kvp in connections)
            {
        ...
        static void OnConnected(int connectionId)
        {
            ...
            {
                // add connection
                ConnectionToClient conn = new ConnectionToClient(connectionId);
                OnConnected(conn);
            }
            else
            {
        ...
        internal static void OnConnected(ConnectionToClient conn)
        {
        ...
        static void OnDisconnected(int connectionId)
        {
            ...
            if (connections.TryGetValue(connectionId, out ConnectionToClient conn))
            {
                conn.Disconnect();
        ...
        internal static void SpawnObject(GameObject obj, NetworkConnection ownerConnection)
        {
            ...
            identity.Reset();
            identity.connectionToClient = (ConnectionToClient)ownerConnection;
