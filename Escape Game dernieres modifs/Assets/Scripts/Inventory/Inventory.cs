...
public class Inventory : MonoBehaviour
{
    ...
    private bool hasPassed1 = false;
    private bool hasPassed2 = false;
    ...
    public void modifsPositionObject(GameObject itemObject){
        if(SceneManager.GetActiveScene().name == "the last revelation 1")
        {
            ...
            Debug.Log(itemObject.GetComponent<AudioSource>().enabled);
            if((itemObject.name == "BookPause" || itemObject.name == "BookBudget") && itemObject.GetComponent<AudioSource>().enabled){
                itemObject.transform.Rotate(new Vector3(0,0,180));
                Debug.Log("passe");
            }
            if((itemObject.name == "BookVersionnage" || itemObject.name == "BookCahier" || itemObject.name == "BookReunion") && itemObject.GetComponent<AudioSource>().enabled){
                itemObject.transform.Rotate(new Vector3(0,0,90));
                Debug.Log("passe");
            }
            
            itemObject.GetComponent<Outline>().enabled = false;
            itemObject.GetComponent<AudioSource>().enabled = false;
        }    
    }
}
